...
class ThreadedCamera:
    def __init__(self, src=0, width=1920, height=1080):
               # Mandatory typed GStreamer string for Jetson Orin Nano
        gst_str = (
        ...
        print(f"\n[DEBUG] GST_STR: {gst_str}")



        print(f"[DEBUG] Opening GStreamer pipeline for camera {src}")
        ...
        if not self.cap.isOpened():
            print(f"[WARN] CSI Pipeline failed, trying V4L2 for camera {src}")
...
def ensure_rgb(img):
    """Convert 2-D grayscale image to BGR if needed."""
...
def main():
    ...
    while True:
        ...
        if DRAW_FACE_MESH:
            ...
            if len(img_face.shape) == 2:
                img_face = cv2.cvtColor(img_face, cv2.COLOR_GRAY2BGR)
